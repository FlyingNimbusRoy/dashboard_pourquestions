...
@section('content')
    <h1 class="text-2xl font-bold mb-4">Upload Gamepack Art</h1>
    ...
    <form action="{{ route('dashboard.gamepacks.upload.store') }}" method="POST" enctype="multipart/form-data" class="mb-6">
        @csrf
    ...
    <h2 class="text-xl font-semibold mb-2">Current Pack Art</h2>
        ...
        @forelse($images as $image)
            <div class="w-32 flex flex-col items-center">
                <div class="w-32 h-32 border rounded-lg shadow overflow-hidden bg-gray-100">
                    <img src="{{ asset('img/gamepacks/' . $image->getFilename()) }}"
                         alt="Gamepack Art"
                ...
                <form action="{{ route('dashboard.gamepacks.upload.destroy', $image->getFilename()) }}"
                      method="POST"
                      class="mt-1 w-full">
                    @csrf
                    @method('DELETE')
