...
@section('content')
    <h1 class="text-2xl font-bold mb-4">Upload Character Image</h1>
    ...
    <form action="{{ route('dashboard.characters.upload.store') }}" method="POST" enctype="multipart/form-data" class="mb-6">
        @csrf
        ...
        @foreach($images as $image)
            <div class="w-32 flex flex-col items-center">
                <div class="w-32 h-32 border rounded-lg shadow overflow-hidden bg-gray-100">
                    <img src="{{ asset('img/characters/' . $image->getFilename()) }}"
                         alt="Character"
                         class="w-full h-full object-cover">
                </div>

                <!-- Delete Button -->
                <form action="{{ route('dashboard.characters.upload.destroy', $image->getFilename()) }}"
                      method="POST"
                      class="mt-2 w-full">
                    @csrf
                    @method('DELETE')
